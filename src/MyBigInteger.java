/*
 * STUDENT NAME : Murat Polat STUDENT ID : 260738186
 * 
 * If you have any issues that you wish the T.A.s to consider, then you should list them here. If you discussed on the
 * assignment in depth with another student, then you should list that student's name here. We insist that you each
 * write your own code. But we also expect (and indeed encourage) that you discuss some of the technical issues and
 * problems with each other, in case you get stuck.
 * 
 * 
 */
...
public class MyBigInteger implements Comparable<MyBigInteger> { // We will cover what "implements Comparable" means
  ...
  /*
   * 
   * If the number has N digits, then it is represented is a polynomial: coefficients[N-1] base^{N-1} + ...
   * coefficients[1] base^{1} + coefficients[0] where base has a particular value and the coefficients are in {0, 1, ...
   * base - 1}
   * 
   * For any base and any positive integer, the representation of that positive integer as a sum of powers of that base
   * is unique.
   * 
   * We require that the coefficient of the largest power is non-zero. For example, '354' is a valid representation
   * (which we call "three hundred fifty four") but '0354' is not.
   * 
   */
  ...
  MyBigInteger(String numberAsString, int base) throws Exception {
    ...
    for (int indx = 0; indx < len; indx++) {
      ...
      /*
       * ascii value of symbol '0' is 48, symbol '1' is 49, etc. So, to get the numerical value of '2', we subtract the
       * character value of '0' (48) from the character value of '2' (50).
       */
  ...
  /*
   * This Constructor allows the input to be an int. The maximum value can therefore be 2^31-1. Note that the method
   * assumes the input i is written in base 10. It then converts that value to a particular base.
   */
  ...
  /*
   * Normally we would not need this method, since coefficients are meant to be hidden from the user. The reason for
   * having the method is that the Grader can use it to examine the number. Another way to do this would have been to
  ...
  public MyBigInteger plus(MyBigInteger second) throws Exception {
    ...
    /*
     * The plus method must not affect the numbers themselves. So let's just work with a copy (a clone) of the numbers.
     */
    ...
    /*
     * If the two numbers have a different polynomial order then pad the smaller one with zero coefficients.
     */
    ...
    /*
     * 'firstClone' and 'secondClone' have the same size. We add the coefficients term by term. If the last coefficient
     * yields a carry, then we add one more term with the carry.
     */
  ...
  /*
   * Slow multiplication algorithm. 'this' is the multiplicand i.e. a*b = a+a+a+...+a (b times) where a is multiplicand
   * and b is multiplier
   */
  ...
  public MyBigInteger slowTimes(MyBigInteger multiplier) throws Exception {
    ...
    for (MyBigInteger counter = new MyBigInteger(0, this.base); counter.compareTo(multiplier) < 0; counter =
        counter.plus(one)) {
      prod = prod.plus(this);
    }
    return prod;
  ...
  /*
   * The times method is NOT be the same as what you learned in grade school which uses a temporary 2D table with space
   * proportional to the square of the number of coefficients in the operands, i.e. N^2. The method here accumulates the
   * result by adding each row of the table as it is computed. This method uses space that is proportional to the number
   * of coefficients N. The multiplication algorithm will still take time O(N^2) however.
   * 
  ...
  /*
   * The minus method computes this.minus(b) where 'this' is a, and a > b. If a < b, then it throws an exception.
   * 
   * The solution below uses variable names first and second rather than a and b. So we are computing
   * first.minus(second)
   */
  ...
  public MyBigInteger minus(MyBigInteger second) throws Exception {
    ...
    /*
     * The minus method is not supposed to change the numbers. But the grade school algorithm sometimes requires us to
     * "borrow" from a higher coefficient to a lower one. So we work with a copy (a clone) instead.
     */
    ...
    while (i < first.coefficients.size()) {
      ...
      /*
       * Start from the least significant digit. For each i, check if 'second' has a term at position i. If yes, then
       * need to take the difference at this position. That's where the bulk of work is. If no, then the coefficient at
       * second is treated as 0 and just copy the i'th coefficient from first to the result.
       */
      ...
      if (i < second.coefficients.size()) {
        ...
        if (diffCoef >= 0)
          ...
          /*
           * .. then we need to borrow from the next coefficient. But if the next coefficient holds a '0', then we need
           * a sequence of borrows.
           * 
           * For example, suppose we are computing 30001 - 6. Then we essentially treat 30001 as 11 + 90 + 900 + 9000 +
           * 2000. So we would want to rewrite the list of coefficients as (11, 9, 9, 9, 2).
           */
          int j = i;
  ...
  public MyBigInteger slowdividedBy(MyBigInteger divisor) throws Exception {
    ...
    MyBigInteger quotient = new MyBigInteger(this.coefficients.size(), base);
  ...
  /*
   * The dividedBy method divides 'this' by 'divisor' i.e. this.dividedBy(divisor) It returns the quotient and ignores
   * the remainder.
   */
  ...
  public MyBigInteger dividedBy(MyBigInteger divisor) throws Exception {
    ...
    if (divisor.base == 0) {
      throw new Exception("Base cannot be zero");
    }
    if (divisor.coefficients.size() == 0) {
      throw new Exception("Divisor cannot be null");
    }
    int totalBase10 = 0;
    MyBigInteger initOperand;
    for (int i = 0; i < divisor.coefficients.size(); i++) {
      totalBase10 = totalBase10 + divisor.coefficients.get(i) * this.base;
    }
    if (totalBase10 == 0) {
    ...
    MyBigInteger operand = new MyBigInteger(0, this.base);
    boolean init = true; //avoids error
    int whilerun = 0;
    int lostDigit = 0;
    boolean gone = false;
    int initSize = 0;
    ...
    while (this.compareTo(divisor) >= 0) {
      // System.out.println("dividend is " + this);
      
      for (int i = this.coefficients.size() - divisor.coefficients.size(); i < this.coefficients.size(); i++) { // gets
                                                                                                                // the
                                                                                                                // operand
                                                                                                                // for
                                                                                                                // subtraction
        if (init) {
          operand.coefficients.set(0, this.coefficients.get(i));
          init = false;
        } else {
          operand.coefficients.add(this.coefficients.get(i));
        }
      }
   //   System.out.println("Operand is " + operand);
      while (operand.compareTo(divisor) < 0 && (this.coefficients.size() - divisor.coefficients.size() > 0)) { // in
                                                                                                               // case
                                                                                                               // divisor.size()
                                                                                                               // digits
                                                                                                               // isn't
                                                                                                               ...
                                                                                                               // for
                                                                                                               // subtraction
        operand.coefficients.add(0, this.coefficients.get(this.coefficients.size() - divisor.coefficients.size() - 1));
        if (gone) {
          quotient.coefficients.add(0,0);
         
        }
      }
      initSize = this.coefficients.size();
      initOperand = operand.clone();
      whilerun = 0;
      while (operand.compareTo(divisor) >= 0) {// subtracts until operand is less than divisor
        if (whilerun == 0) {
          quotient.coefficients.add(0, 0);
          whilerun++;
        }
        operand = operand.minus(divisor);
        quotient.coefficients.set(0, quotient.coefficients.get(0) + 1);
      ...
      while (initOperand.coefficients.size() != operand.coefficients.size()) {
        operand.coefficients.add(operand.coefficients.size(), 0);
      }
      for (int p = this.coefficients.size() - operand.coefficients.size(); p < this.coefficients.size(); p++) { // sets
                                                                                                                // the
                                                                                                                // remainder
                                                                                                                // as
                                                                                                                // the
                                                                                                                ...
                                                                                                                // of
                                                                                                                // this
        this.coefficients.set(p, operand.coefficients.get(k));
        k++;

      }
   if (operand.coefficients.get(operand.coefficients.size()-1) == 0) {
        gone = true;
      }
      while ((operand.coefficients.size() > 1) & (operand.coefficients.get(operand.coefficients.size() - 1) == 0)) {
        operand.coefficients.remove(operand.coefficients.size() - 1);
      }
      if (operand.coefficients.size() == divisor.coefficients.size()) {
        gone = false;
      }
      /*
       * I copied remove zeroes code from the minus method above, modified it to fit the division algorithm
       */
//      System.out.println("operand " + operand);
      lostDigit = 0;
      while ((this.coefficients.size() >= divisor.coefficients.size())
          & (this.coefficients.get(this.coefficients.size() - 1) == 0)) {
        if ( operand.coefficients.get(0)==0 && lostDigit>0) {
          quotient.coefficients.add(0,0);
        }
...
//      System.out.println("init operand is " + initOperand);
//       System.out.println("Quotient is " + quotient);
//       System.out.println("divisor is " + divisor);
//       System.out.println("operand is " + operand);
//       System.out.println("run is " + run);

      // System.out.println("lostdigit " + lostDigit);
      run++;

      operand.coefficients.clear();

    }
    /*
     * I copied remove zeroes code from the minus method above.
     */
    while ((quotient.coefficients.size() > 1) & (quotient.coefficients.get(quotient.coefficients.size() - 1) == 0)) {
  ...
  /*
   * The convert method converts between two bases that are each in {2, ..., 10} The convert method specifies the new
   * base that you are converting into. The subtlety of the convert method is that it uses operations in this.base This
   * can be tricky since if you are converting into a higher base then you have to consider that this.dividedBy() is
   * performed in this.base.
   * 
   */
  ...
  public MyBigInteger convert(int newBase) throws Exception {
    ...
    // ADD YOUR CODE BELOW HERE
    if (newBase < 2) {
      throw new Exception("Base cannot be less than 2");
    }
    if (newBase > 10) {
      throw new Exception("Base cannot be more than 10");
    }
    if (newBase == this.base) {
      result = this.clone();
    } else {
      int decimal = 0;
      ArrayList<Integer> decimalStorage = new ArrayList<Integer>();
      int tmp;
      int surplus;
      for (int k = 0; k < this.coefficients.size(); k++) {
//        System.out.println("base is " + this.base);
//        System.out.println("coeff is " + k);
//        System.out.println("this(k) is " + this.coefficients.get(k));
//        System.out.println("total is " + (Math.pow(this.base, k)* this.coefficients.get(k)));
      decimal += ((Math.pow(this.base, k)* this.coefficients.get(k)));
//      System.out.println("k is " +k);
//      System.out.println("base is " + this.base);
//      System.out.println("coefficient is " + this.coefficients.get(k));
//      System.out.println("toadd is " + ((Math.pow(this.base, k)* this.coefficients.get(k))) );
...
//      System.out.println("ok ");
      while (decimal > 0) {
        tmp = decimal /  newBase;
        surplus = decimal - (tmp * newBase);
        result.coefficients.add(surplus);
  ...
  public ArrayList<MyBigInteger> primeFactors() throws Exception {
    ...
    MyBigInteger zero = new MyBigInteger(0, this.base);
    for (int k = 0; k < this.coefficients.size(); k++) {
      number += (Math.pow(this.base,k)) * this.coefficients.get(k);
    }
    number = (int) Math.sqrt(number);
    ArrayList<MyBigInteger> primeFactors = primesToN(number, this.base);
    System.out.println("number " + number);
    System.out.println(primeFactors.get(0));
    for (int startN = 0; startN < primeFactors.size(); startN++) {
      if (this.mod(primeFactors.get(startN)).compareTo(zero) == 0) {
        factors.add(primeFactors.get(startN));
        this.dividedBy(primeFactors.get(startN));
        startN--;
      }
    }
  ...
  /*
   * The a.compareTo(b) method returns -1 if a < b, it returns 0 if a == b, and it returns 1 if a > b.
   * 
  ...
  public int compareTo(MyBigInteger second) {
    ...
    /*
     * Assume maximum degree coefficient is non-zero (except for case that one of the numbers is 0). Then, if two
     * numbers have different maximum degree, it is easy to decide which is larger.
     */
    ...
    if (diff < 0)
      ...
      /*
       * If two numbers have the same maximum degree, then it is a bit trickier to decide which number is larger. You
       * need to compare the coefficients, starting from the largest and working toward the smallest until you find
  ...
  /*
   * computes 'this' * base^n Note that it modifies the number, so if you don't want that, then clone the number first.
   */
